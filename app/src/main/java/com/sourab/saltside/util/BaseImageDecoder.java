...
public class BaseImageDecoder implements ImageDecoder {
	 ...
	 */
	public BaseImageDecoder(Context context,boolean loggingEnabled) {
	...
	public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
		...
		try {
			...
			if(scaleType == ImageScaleType.EXACTLY)
			{
				...
				decodedBitmap = scaleImage(imageInfo.imageSize.getHeight(), imageInfo.imageSize.getWidth(), imageStream);
			}
		} finally {
	...
	private Bitmap scaleImage(int height , int width , InputStream imageStream) throws NoSuchElementException {
		Bitmap scaledBitmap;
		int bounding = dpToPx(CONSTANTS.THUMB_SIZE_USER_CONTENT);
		...
		if(xScale > yScale)
		{
			...
			scaledBitmap = Bitmap.createBitmap(BitmapFactory.decodeStream(imageStream), 0, 0, width, height, matrix, true);
		}
		else
		{
			scaledBitmap = Bitmap.createScaledBitmap(BitmapFactory.decodeStream(imageStream), bounding,bounding, true);
		}
		return scaledBitmap;
	}
